...
### `trailing_take_profit(state, current_price, x, futures=False, multiplier=1.0) -> bool`

浮动止盈。从开仓起持续追踪价格峰值，一旦从峰值回撤超过 `x`（比例），立即全仓平仓。
...
峰值存储在 `state['_trail_peak']`，触发时自动清除。

```python
# 多头：从峰值回撤30%则止盈
trailing_take_profit(state, current_price=price, x=0.30)
```
